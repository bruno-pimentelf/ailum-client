...
export function GeralTab() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      reminder48hEnabled: tenant.reminder48hEnabled ?? false,
      reminder24hEnabled: tenant.reminder24hEnabled ?? true,
      reminder2hEnabled: tenant.reminder2hEnabled ?? false,
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      await update.mutateAsync({
        ...
        reminder48hEnabled: form.reminder48hEnabled,
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="rounded-xl border border-border/50 bg-card/30 p-5 space-y-4">
          ...
          <p className="text-[11px] text-muted-foreground/70">
            Mensagens enviadas automaticamente antes de consultas. Edite os textos nos templates com as chaves <code className="text-[10px] bg-muted/30 px-1 py-0.5 rounded">reminder_48h</code>, <code className="text-[10px] bg-muted/30 px-1 py-0.5 rounded">reminder_24h</code>, <code className="text-[10px] bg-muted/30 px-1 py-0.5 rounded">reminder_2h</code> e <code className="text-[10px] bg-muted/30 px-1 py-0.5 rounded">reminder_1h</code>.
          </p>

          <div className="space-y-2">
            {([
              { key: "reminder48hEnabled" as const, label: "48h antes", desc: "Enviado ~48h antes da consulta" },
              { key: "reminder24hEnabled" as const, label: "24h antes", desc: "Enviado ~24h antes da consulta" },
              { key: "reminder2hEnabled" as const, label: "2h antes", desc: "Enviado ~2h antes da consulta" },
              { key: "reminder1hEnabled" as const, label: "1h antes", desc: "Enviado ~1h antes da consulta" },
            ]).map((r) => (
              <div key={r.key} className="flex items-center justify-between rounded-lg border border-border/40 bg-muted/15 px-3 py-2.5">
                <div>
                  <p className="text-[12px] font-medium text-foreground">Lembrete {r.label}</p>
                ...
                <button
                  ...
                  className={`relative h-6 w-10 shrink-0 rounded-full border-2 transition-colors cursor-pointer ${
                    form[r.key] ? "border-accent bg-accent/20" : "border-border bg-muted/30"
                  }`}
                >
                  <span className={`absolute top-0.5 h-4 w-4 rounded-full transition-all ${
                    form[r.key] ? "left-5 bg-accent" : "left-0.5 bg-muted-foreground/50"
                  }`} />
                </button>
              </div>
            ))}
