...
## 5) Marcar como lida (via API backend)

Use os endpoints abaixo para marcar leitura.  
O `onSnapshot` continua sendo a fonte de verdade da UI (badge/lista atualizam automaticamente).

### 5.1 Marcar uma notificação

`PATCH /v1/tenant/notifications/:notificationId/read`
...
- `read` é opcional (default `true`).
- Se `notificationId` não existir no tenant: `404`.

### 5.2 Marcar todas como lidas

`PATCH /v1/tenant/notifications/read-all`
...
- `maxDocs` opcional (default `1000`, máximo `5000`).
- O backend atualiza em lote somente docs `read=false`.
